...
export default function EmployeesPage() {
  ...
  const fetchData = () => {
    ...
    // Fetch Departments
    fetch(`${getApiBaseUrl()}/admin/departments`, { headers })
      .then(res => res.json())
      .then(data => {
  ...
  const handleAddEmployee = async (e: React.FormEvent, force: boolean = false) => {
    ...
    try {
      const token = localStorage.getItem("aegis_access_token") || localStorage.getItem("aegis_token");
      const res = await fetch(`${getApiBaseUrl()}/admin/users?force_transfer=${force}`, {
        method: "POST",
        headers: {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("aegis_access_token") || localStorage.getItem("aegis_token");
      const res = await fetch(`${getApiBaseUrl()}/admin/users/${selectedEmployee.id}/password?new_password=${encodeURIComponent(newPassword)}`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`
        }
  ...
  const handlePromoteUser = async () => {
    try {
      const token = localStorage.getItem("aegis_access_token") || localStorage.getItem("aegis_token");
      const res = await fetch(`${getApiBaseUrl()}/admin/users/${selectedEmployee.id}/promote`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`
        }
  ...
  const handleDeleteUser = async () => {
    try {
      const token = localStorage.getItem("aegis_access_token") || localStorage.getItem("aegis_token");
      const res = await fetch(`${getApiBaseUrl()}/admin/users/${selectedEmployee.id}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`
        }
  ...
  const handleResetPasswordDirect = async (userId: number, pw: string) => {
    try {
      const token = localStorage.getItem("aegis_access_token") || localStorage.getItem("aegis_token");
      const res = await fetch(`${getApiBaseUrl()}/admin/users/${userId}/password?new_password=${encodeURIComponent(pw)}`, {
        method: "PUT",
  ...
  const handlePromoteDirect = async (userId: number, name: string) => {
    try {
      const token = localStorage.getItem("aegis_access_token") || localStorage.getItem("aegis_token");
      const res = await fetch(`${getApiBaseUrl()}/admin/users/${userId}/promote`, {
        method: "PUT",
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleRoleChangeDirect = async (userId: number, newRole: string, name: string) => {
    try {
      const token = localStorage.getItem("aegis_access_token") || localStorage.getItem("aegis_token");
      const res = await fetch(`${getApiBaseUrl()}/admin/users/${userId}/role?role=${newRole}`, {
        method: "PUT",
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleToggleStatusDirect = async (userId: number, currentStatus: string, name: string) => {
    ...
    try {
      const token = localStorage.getItem("aegis_access_token") || localStorage.getItem("aegis_token");
      const res = await fetch(`${getApiBaseUrl()}/admin/users/${userId}/status?status=${nextStatus}`, {
        method: "PUT",
        headers: { "Authorization": `Bearer ${token}` }
      });
      if (res.ok) {
