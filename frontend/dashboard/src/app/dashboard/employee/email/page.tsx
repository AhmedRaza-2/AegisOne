...
export default function EmployeeEmailSecurityPage() {
  ...
  const loadAnalytics = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8000/analytics/email?period=${period}&scope=employee`, {
  ...
  const filteredScans = (data?.scans || []).filter(s => {
    ...
    return s.url.toLowerCase().includes(q) || s.verdict.toLowerCase().includes(q);
  });
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-6 rounded-2xl border border-slate-800/80 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-2 bg-slate-950/80 p-1.5 rounded-xl border border-slate-800">
          ...
          <button
            ...
            className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-900 rounded-lg transition-all ml-1"
            title="Refresh Data"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          className="bg-slate-900/50 p-5 rounded-2xl border border-slate-800/80 relative overflow-hidden">
          ...
          <div className="text-3xl font-extrabold text-white mt-3">{data?.summary.total_scanned || 0}</div>
          <p className="text-xs text-slate-500 mt-1">Verified email instances</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="bg-slate-900/50 p-5 rounded-2xl border border-slate-800/80 relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Phishing Blocked</span>
            <div className="p-2 bg-red-500/10 rounded-lg text-red-400"><ShieldAlert className="w-4 h-4" /></div>
          </div>
          <div className="text-3xl font-extrabold text-red-400 mt-3">{data?.summary.phishing || 0}</div>
          <p className="text-xs text-slate-500 mt-1">High-confidence malicious targets</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="p-2 bg-amber-500/10 rounded-lg text-amber-400"><AlertTriangle className="w-4 h-4" /></div>
          </div>
          <div className="text-3xl font-extrabold text-amber-400 mt-3">
            {((data?.summary.threat_rate || 0) * 100).toFixed(1)}%
          </div>
          <p className="text-xs text-slate-500 mt-1">Ratio of non-safe email content</p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          className="bg-slate-900/50 p-5 rounded-2xl border border-slate-800/80 relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Avg Risk Index</span>
            <div className="p-2 bg-emerald-500/10 rounded-lg text-emerald-400"><ShieldCheck className="w-4 h-4" /></div>
          </div>
          <div className="text-3xl font-extrabold text-emerald-400 mt-3">
            {data?.summary.average_risk_score || 0}
            <span className="text-sm font-normal text-slate-400"> / 100</span>
          </div>
          <p className="text-xs text-slate-500 mt-1">Mean AI risk score</p>
        </motion.div>
      </div>
      ...
      <div className="bg-slate-900/60 rounded-2xl border border-slate-800/80 overflow-hidden">
        ...
        <div className="p-5 border-b border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-white">Email Scan Telemetry</h2>
            <p className="text-xs text-slate-400">Detailed security logs of emails processed by Aegis AI.</p>
          </div>
                  ...
                  return (
                    <tr key={scan.id} className="hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-3.5 px-5 font-bold">
                        <span className={isPhish ? "text-red-400" : isSusp ? "text-amber-400" : "text-emerald-400"}>
                          {score}%
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-5 capitalize text-slate-400">
                        {scan.decision || "allow"}
                      </td>
                      ...
                      <td className="py-3.5 px-5 text-right">
                        <button
                          ...
                          className="px-3 py-1.5 bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-300 border border-indigo-500/30 rounded-lg text-xs font-semibold transition-all inline-flex items-center gap-1.5"
                        >
        ...
        {selectedScan && (
          ...
          >
            <motion.div
              ...
              className="bg-slate-900 border border-indigo-500/30 rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl text-white relative"
            >
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-base">Explainable AI Analysis</h3>
                    <p className="text-xs text-slate-400">{selectedScan.url}</p>
                  </div>
                </div>
                <button onClick={() => setSelectedScan(null)} className="text-slate-400 hover:text-white text-sm">✕</button>
              </div>

              {/* Score & Verdict Banner */}
              <div className="flex items-center gap-4 p-4 rounded-xl bg-slate-950/80 border border-slate-800">
                <div className="w-14 h-14 rounded-full border-4 border-indigo-500/50 flex flex-col items-center justify-center bg-black/40 shrink-0">
                  <span className="text-lg font-black text-indigo-400">{selectedScan.risk_score}%</span>
                </div>
                <div>
                  <div className="text-sm font-bold capitalize text-white">Verdict: {selectedScan.verdict}</div>
              ...
              <div className="space-y-2">
                <span className="text-xs font-bold uppercase tracking-wider text-indigo-400">Flagged Risk Indicators</span>
                {selectedScan.top_factors && selectedScan.top_factors.length > 0 ? (
                  ...
                  <div className="p-3 bg-slate-950/50 rounded-lg text-xs text-slate-400">
              ...
              <button
                onClick={() => setSelectedScan(null)}
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 font-bold rounded-xl text-xs transition-colors"
              >
                Close Analysis
              </button>
            </motion.div>
