...
import { scanURL, scanPageText, scanURLBatch, scanEmail, checkHealth } from "./scanner.js";
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  if (info.menuItemId === "aegis-scan-link" && info.linkUrl) {
    ...
    if (tabId) {
      chrome.tabs.sendMessage(tabId, {
        type: MSG.FULL_PAGE_SCAN,
      }).catch(() => {});
    }
  } else if (info.menuItemId === "aegis-scan-image" && info.srcUrl) {
    const result = await scanURL(info.srcUrl);
    if (tabId) {
      chrome.tabs.sendMessage(tabId, {
        type: MSG.RIGHT_CLICK_SCAN,
        url: info.srcUrl,
